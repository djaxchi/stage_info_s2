...
def patch_up(m):  # renvoie une matrice dont le graphe de buneman est complet
    g = m.buneman_graph()
    ...
    while c < card_E:  # permet de prendre en compte l'heritage d'un triangle
        ...
        for x in E:
            if (
                x and x[::-1] not in g.edges
            ):  # on regarde tout aretes qui n'est pas dans le graphe
                ...
                if (
                    len(triangle) != 0
                ):  # on verifie si l'arete manquante n'est pas imposé par un triangle
                    for nodes in triangle:
                        if dico[nodes, x[0]][0] != dico[nodes, x[1]][0]:
                            ...
                            for ligne in dico_remplacent:
                                modifie_ligne(
                                    m, dico_remplacent[ligne], x, ligne
                                )  # on place l'arete imposée par un triangle
                            c = 0
            else:
                c += 1
    g = m.buneman_graph()
    ...
    for cc in P:
        ...
        for o_candidat in liste_omega_possible:
            couple_dico_cdo_test = correction_lignes(
                m, cc, o_candidat
            )  # ici on test le meilleur omega possible pour un edge non existant (on aurait pu choisir le meilleur pour une solution plus optimale)
            ...
            if couple_omega_cdo_test[1] <= cdo:
                ...
                couple_dico_omega_choisi = (
                    couple_dico_cdo_test[0],
                    couple_omega_cdo_test[0],
                )
...
def correction_lignes(
    m, c, o
):  # matrice,couple,omega #permet de renvoyer les changement optimal a faire sur chaque ligne pour un omega donné
    ...
    dico_remplacent = (
        {}
    )  # dico qui repertotrie tout les remplacent de chaque ligne pour un omega
    for ligne in L:
        couple_remplacant_cdr = trouve_remplacant(
            m, c, o, ligne
        )  # ici on appel une fonction qui nous renvoie le remplacant ideal et sa destruction
        if couple_remplacant_cdr[0] != None:
            cdo += couple_remplacant_cdr[
                1
            ]  # chaque ligne cause une certaine destruction
            modifie_ligne(
                m_test, couple_remplacant_cdr[0], o, ligne
            )  # on effectue le changement de ligne pour quu'il soit pris en compte pour les prochaines lignes
            dico_remplacent[ligne] = couple_remplacant_cdr[
                0
            ]  # on note le remplacent pour pouvoir le faire remonter
        else:
...
def trouve_remplacant(m, c, o, ligne):
    ...
    for candidat in remplacement_possible:
        ...
        if (
            c[0] or c[1] not in m_test.proper()
        ):  # l'ajout du candidat rend des colonnes impropre donc le candidat est eliminé
            cdr_test = cdr + 1
        if (
            modifie_ligne(m_test, candidat, c, ligne) == None
        ):  # l'ajout réel du candidat rend des colonnes impropre donc elimination du candidat
            cdr_test = cdr + 1
        for i in range(
            len(m.matrix[ligne])
        ):  # on verifie les difference entre la matrice changé et la matrice initial
...
def modifie_ligne(
    m, remplacant, c, ligne
):  # fonction qui sert a inséré un couple remplacant sans affecter les omegas d'origine
    ...
    for a in voisin_x:
        if a not in voisin_x1:
            ...
            c = (a, c[0])
            modifie_ligne(m, remplacant, c, ligne)
    for b in voisin_y:
